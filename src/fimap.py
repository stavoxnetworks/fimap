...
def show_help(AndQuit=False):
    ...
    print("                                 file directories. Eats an extra request for every 'cd' command.")
...
if __name__ == "__main__":
    config["p_url"] = None
    # 0=single ; 1=mass ; 2=google ; 3=crawl ; 4=autoawesome ; 5=bing
    config["p_mode"] = 0
    config["p_list"] = None
    config["p_verbose"] = 2
    ...
    try:
        ...
        optlist, args = getopt.getopt(
            sys.argv[1:], "u:msl:v:hA:gq:p:sxXHw:d:bP:CIDTM:4R:B", longSwitches)
        ...
        if (lockFound):
            if (config["force-run"] == True):
                print(
                    "Another fimap instance is running! But you requested to ignore that...")
            else:
                print("Another fimap instance is already running!")
                print(
                    "If you think this is not correct please delete the following file:")
            ...
            lockfile = tempfile.NamedTemporaryFile(
                prefix="fimap_", suffix="_lockfile")
    ...
    if (doUpdateDef):
        ...
        f.close()
        generic_xml_ver_online = tools.getAttributeFromFirstNode(
            tmpFile, "revision")

        if (generic_xml_ver < generic_xml_ver_online):
            print("'generic.xml' (v.%s) is older than the online version (v.%s)!" % (
                generic_xml_ver, generic_xml_ver_online))
        ...
        for name, langclass in list(langsets.items()):
            ...
            if (xml_content != None):
                ...
                version_online = tools.getAttributeFromFirstNode(
                    tmpFile, "revision")
                # Get installed version.
                version = langclass.getVersion()
                if (version < version_online):
                    print("'%s' (v.%s) is older than the online version (v.%s)!" % (
                        fname, version, version_online))
    ...
    if (doInstallPlugins):
        ...
        for k, (l, n, v, u) in list(choice.items()):
            ...
            if (instver == None):
                boxarr.append(
                    "[%d] %s - At version %d not installed." % (k, l, v))
            elif (instver < v):
                boxarr.append(
                    "[%d] %s - At version %d has an UPDATE." % (k, l, v))
            else:
                boxarr.append(
                    "[%d] %s - At version %d is up-to-date and installed." % (k, l, v))
            ...
            if (plugin != None):
                ...
                try:
                    ...
                    if (os.path.exists(pluginxml)):
                        ...
                        ver = pluginman.getPluginVersion(
                            info.getStartupClass())
                        if (ver != None):
                            inp = ""
                            if (ver > info.getVersion()):
                                inp = input(
                                    "Do you really want to downgrade this plugin? [y/N]")
                            elif (ver == info.getVersion()):
                                inp = input(
                                    "Do you really want to reinstall this plugin? [y/N]")
                        ...
                        print("Plugin '%s' installed successfully!" %
                              (info.getName()))
                    else:
                        print("Plugin doesn't have a plugin.xml! (%s)" %
                              pluginxml)
                        sys.exit(1)
    ...
    if (doPluginsShow):
        ...
        if (len(plugins) > 0):
            for plug in plugins:
                print("[Plugin: %s] by %s (%s)" % (plug.getPluginName(),
                                                   plug.getPluginAutor(), plug.getPluginEmail()))
        else:
            print("No plugins :T")
        sys.exit(0)
    ...
    if (doMergeXML):
        ...
        print("%d new vulnerabilitys added from %d new domains." %
              (newVulns, newDomains))
        sys.exit(0)
    ...
    try:
        if (config["p_mode"] == 0):
            ...
            print("Crawler is harvesting URLs from start URL: '%s' with depth: %d and writing results to: '%s'" % (
